...
// Text Reading Page - Text-based tarot reading mode
...
import { FollowupChatTipModal } from '@/components/reading/FollowupChatTipModal'
...
import { isReadingMainData, isExplanationMessageData } from '@/types'
...
export default function TextReadingPage() {
  ...
  const [tipAlreadyDismissedInDB, setTipAlreadyDismissedInDB] = useState(false)
  ...
  // Fetch user tip state on mount
  useEffect(() => {
    const fetchUserTipState = async () => {
      try {
        const response = await fetch('/api/user/get-user')
        if (response.ok) {
          const data = await response.json()
          if (data.user?.tipsShown?.followupChatTip) {
            setTipAlreadyDismissedInDB(true)
          }
        }
      } catch (error) {
        console.error('Failed to fetch user tip state:', error)
        // Fail silently - tip will show if unable to fetch
      }
    }
  ...
  // Check if tip should be shown (after 3 followup messages)
  useEffect(() => {
    if (
      state.type === 'followUps' &&
      session &&
      !hasTipBeenShown &&
      !tipAlreadyDismissedInDB &&
      !showTipModal
    ) {
      // Count user messages in followUps phase
      // Find the first ReadingMainData message (the initial reading)
      const readingMessageIndex = session.messages.findIndex(
        (msg) => isReadingMainData(msg.data)
      )

      // If no reading found yet, don't show tip
      if (readingMessageIndex === -1) {
        return
      }

      // Count only user messages that come AFTER the reading message
      const userFollowupMessageCount = session.messages
        .slice(readingMessageIndex + 1) // Only messages after the reading
        .filter((msg) => msg.isUser && typeof msg.data === 'string')
        .length

      // Show tip after 3 or more followup messages
      if (userFollowupMessageCount >= 3) {
        setShowTipModal(true)
      }
    }
  }, [state.type, session, hasTipBeenShown, tipAlreadyDismissedInDB, showTipModal])
  ...
  const handleClarificationMessage = async (message: string) => {
    ...
    try {
      // Get the last AI response ID for context (excluding explanation messages from Why feature)
      const lastResponseId = session.messages
        .filter((msg) => !msg.isUser && msg.responseId && !isExplanationMessageData(msg.data))
        .pop()?.responseId
  ...
  const handleWhyRequest = async (expandedText: string, responseId: string): Promise<void> => {
    if (!session) return
  ...
  const handleTipDismiss = async () => {
    ...
    // Update database
    try {
      const response = await fetch('/api/user/update-tip-state', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tipType: 'followupChatTip' }),
      })

      if (response.ok) {
        setTipAlreadyDismissedInDB(true)
      } else {
        console.error('Failed to update tip state in database')
      }
    } catch (error) {
      console.error('Error updating tip state:', error)
      // Fail silently - tip won't show again in current session anyway
    }
  }
  ...
  return (
    <div className={wrapperClassName}>
      {renderContent()}
      <FollowupChatTipModal isOpen={showTipModal} onClose={handleTipDismiss} />
    </div>
  )
}
